...
export function populateFixingActions(dataTable, chains, log) {
  ...
  for (const chain of chains) {
    for (const link of chain) {
      ...
      if (link.branchChain) {
        populateFixingActionsFromChain(dataTable, link.branchChain, log); // Recurse
      }
    }
  }

  const logArray = log.getLog ? log.getLog() : log;
...
function populateFixingActionsFromChain(dataTable, chain, log) {
  // Simple wrapper to recurse into branches if needed
  populateFixingActions(dataTable, [chain], log);
}
